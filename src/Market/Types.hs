...
instance NFData Scalar where
  rnf frac =
    rnf (numerator frac) `seq`
      rnf (denominator frac) `seq`
        frac `seq`
          ()
...
deriveKappaNewtype
  []
  (appConT ''Values ''Asset)
  (ConT ''Prices)
  (ConT ''Portfolio)
deriveKappaNewtype
  []
  (appConT ''ValueDeltas ''Asset)
  (ConT ''PriceDeltas)
  (ConT ''Portfolio)
deriveKappaNewtype
  []
...
deriveConstrainedQuantityInstances
  ...
  ''ShareDelta
  varKeyT
  (appKeyT ''Distribution)
  (appKeyT ''DistributionDelta)
...
instance Ord k => Truncatable (Distribution k) where
  truncateTo res (Distribution shares) =
    Distribution $ share <$> xs''
    where
      xs'' = addToFirst (one - sum xs') xs'
        where
  ...
  autoWith normalizer =
    fmap (Distribution . weightsToShares . remap id) $
      Dh.map Dh.auto $
        Dh.record $
          Dh.field "share" $
            Dh.autoWith normalizer
    where
      weightsToShares :: SparseMap k Double -> SparseMap k Share
      weightsToShares m = Share . realToFraction <$> m'
        where
...
distributionValid (Distribution shares) =
  all (>= zero) shares'
    && foldl (+) zero shares' == one
  where
    shares' = unShare <$> shares
      where
...
distributionDeltaValid (DistributionDelta shareDeltas) =
  foldl (+) zero shareDeltas' == zero
  where
    shareDeltas' = unShareDelta <$> shareDeltas
      where
...
instance Order Fees where
  fs <~ fs' = leqFixed && variable fs <= variable fs'
    where
...
instance Traversable1 TimeSeries where
  sequence1 = fmap TimeSeries . sequence1 . fmap engulf . unTimeSeries
    where
      engulf (t, ax) = (t,) <$> ax
...
zipSeries s1 s2 =
  fromJust $
    seriesFromList $
      go Nothing Nothing (seriesToList s1, seriesToList s2)
  where
    go prev1 prev2 = \case

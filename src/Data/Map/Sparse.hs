...
instance Ord k => ReadMap k v (SparseMap k v) where
  type Key (SparseMap k v) = k
  type Value (SparseMap k v) = v
...
deriving newtype instance
  (Ord k, Default v, LeftModule a v)
  => LeftModule a (SparseMap k v)
deriving newtype instance
  (Ord k, Default v, RightModule a v)
  => RightModule a (SparseMap k v)
deriving newtype instance
  (Ord k, Default v, Module a v)
...
instance
  (Ord k, Default a, Default b, Delta a b)
  => Delta (SparseMap k a) (SparseMap k b)
  where
  delta xs ys = delta <$> xs <.> ys
  sigma xs ys = Constrained.sequenceA $ sigma <$> xs <.> ys

instance
  (Ord k, Default b, Default c, Kappa a b c)
  => Kappa (SparseMap k a) (SparseMap k b) (SparseMap k c)
  where
...
instance
  (Ord k, Arbitrary k, Default v, Arbitrary v)
  => Arbitrary (SparseMap k v)
  where
  arbitrary = SparseMap . DefaultMap def <$> arbitrary
  shrink m = fmap fromList $ shrink $ toList m

instance
  (Ord k, FromDhall k, Default v, FromDhall v)
  => FromDhall (SparseMap k v)
  where
  autoWith normalizer =

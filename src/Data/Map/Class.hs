...
reapplyLeft
  ...
  -> as
  -> bs
reapplyLeft fs as = fromList $ applyEntry <$> toList fs
  where
...
reapplyRight
  ...
  -> as
  -> bs
reapplyRight fs as = fromList $ applyEntry <$> toList as
  where
...
deriveKappaNewtype cxt a b c = do
  ...
  declareInstance
    cxt
    [t|$(conT ''Kappa) $(pure a) $(pure b) $(pure c)|]
    [
      ( 'kappa
      , [xp, yp]
      , [|
...
toListFromListInversion kvs =
  nub keys == keys ==> sort kvs === sort kvs'
  where
    keys = fst <$> kvs
